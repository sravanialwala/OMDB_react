    ...
    render() {
        ...
        return ( 
            <div className='singleMovie text-white bg-dark'>
                ...
                <center className='text-white'>{this.state.SingleResult.Poster === 'N/A'?<img src={noimg} alt={this.state.SingleResult.Title} width='350px' height='300px' style={{borderRadius:'1px'}}/>:<img src={this.state.SingleResult.Poster} alt={this.state.SingleResult.Title} width='350px' height='300px' style={{borderRadius:'1px'}}/>}
                <br/>
                <br/>
               ...
               <table className='mx-0'>
                   <tbody>
                    <tr className='mx-5' style={{verticalAlign:'top'}}><td><b style={{fontSize:'20px'}}>Director </b></td><td> :<i style={{fontSize:'19px'}}><a href={`https://www.google.com/search?q=${this.state.SingleResult.Director}&rlz=1C1CHBD_enIN858IN858&oq=${this.state.SingleResult.Director}&aqs=chrome.0.0l8.1283j0j7&sourceid=chrome&ie=UTF-8`}>{this.state.SingleResult.Director}</a></i></td></tr>
                    <tr className='mx-5' style={{verticalAlign:'top'}}><td><b style={{fontSize:'20px'}}>Writer </b></td><td> :<i style={{fontSize:'19px'}}>{this.state.SingleResult.Writer && this.state.SingleResult.Writer.split(',').map(x =>(<span key={x}><a href={`https://www.google.com/search?q=${x}&rlz=1C1CHBD_enIN858IN858&oq=${x}&aqs=chrome.0.0l8.1283j0j7&sourceid=chrome&ie=UTF-8`}>{x}</a>,</span>))}</i></td></tr>
                    <tr className='mx-5' style={{verticalAlign:'top'}}><td><b style={{fontSize:'20px'}}>Stars </b></td><td> :<i style={{fontSize:'19px'}}>{this.state.SingleResult.Actors && this.state.SingleResult.Actors.split(',').map(x =>(<span key={x}><a href={`https://www.google.com/search?q=${x}&rlz=1C1CHBD_enIN858IN858&oq=${x}&aqs=chrome.0.0l8.1283j0j7&sourceid=chrome&ie=UTF-8`}>{x}</a>,</span>))}</i></td></tr>
                    <tr className='mx-5' style={{verticalAlign:'top'}}><td><b style={{fontSize:'20px'}}>Production </b></td><td> :<i style={{fontSize:'19px'}}><a href={`https://www.google.com/search?q=${this.state.SingleResult.Production}&rlz=1C1CHBD_enIN858IN858&oq=${this.state.SingleResult.Production}&aqs=chrome.0.0l8.1283j0j7&sourceid=chrome&ie=UTF-8`}>{this.state.SingleResult.Production}</a></i></td></tr>
                    </tbody>
               </table>
               <br/>
